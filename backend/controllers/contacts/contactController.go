...
func (contactCtrl *ContactController) DownloadCSV(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		http.Error(w, "User email not found in context", http.StatusBadRequest)
		return
	}
	contactList, err := contactCtrl.contactrepo.GetContacts(userEmail)
	if err != nil {
	...
	filePath := exportDir + time.Now().String() + userEmail + ".csv"
	if err := checkDirExist(exportDir); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
